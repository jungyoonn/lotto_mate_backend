package com.eeerrorcode.lottomate.service;
...
import com.eeerrorcode.lottomate.repository.LottoResultDetailsRepository;
import com.eeerrorcode.lottomate.repository.LottoResultsRepository;
...
public class LottoCrawlerServiceImpl implements LottoCrawlerService{
    ...
    public void crawlAndSave(WebDriver driver, int round){
        ...
        try {
            if (lottoResultsRepository.existsByDrawRound((long) round)) {
            ...
            Integer bonus = Integer.parseInt(driver.findElement(By.cssSelector(".win_result .bonus span.ball_645")).getText());

            LocalDateTime drawDate = null;
            try {
                String rawDate = driver.findElement(By.cssSelector(".win_result .desc")).getText().replaceAll("[^0-9년월일 ]", "").trim();
            ...
            String winnerStr = driver.findElement(By.cssSelector(".tbl_data tr:nth-of-type(2) td:nth-of-type(2)")).getText().replaceAll("[^0-9]", "");
            String amountStr = driver.findElement(By.cssSelector(".tbl_data tr:nth-of-type(2) td:nth-of-type(3)")).getText().replaceAll("[^0-9]", "");
            ...
            lottoResultsRepository.save(returnResults);
    ...
    public void crawlAll() {
        ...
        try {
            ...
            for(int round = 1; round <= latest; round++){
                if(lottoResultsRepository.existsByDrawRound(Long.valueOf(round))){
    ...
    public void crawlLatest() {
        ...
        try {
            ...
            if(lottoResultsRepository.existsByDrawRound((long)latest)){
    ...
    public Integer getLatestRound() {
        ...
        try {
            ...
            log.info("✅ 최신 회차 번호: {}", latest);
            return latest;
    
        } catch (Exception e) {
            log.warn("최신 회차 정보 로딩 실패: {}", e.getMessage());
            return -1;
        } finally {
            driver.quit();
        }
